...
function normaliza ($string){
    $a = 'ŽÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞ
ßàáâãäåæçèéêëìíîïðñòóôõöøùúûýýþÿRr';
    $b = 'zaaaaaaacèéêëiiîïdnoooooouuuuy
...
return $pageURL;
  ...
  function filtrage_url ($element) {
   $retour = true;
   //La chaîne ne doit pas commencer par ":"
   if (preg_match("#^:#",$element)){
   $retour = False;
   }
   // suppression des valeurs inférieur à 5 caractères
   if (strlen(utf8_decode($element))<= 5){
   $retour = False;
   }
   return $retour;
...
//echo curPageURL();  // Permet de récupérer l'url de la page
                  ...
                  $somecontent = generation($siteliste[$index],$filename);
                                 if ($somecontent != null){
                                    if (Writefile($filename, $somecontent) == true ){
                                      ...
                                      $index=$_GET['index']+1;
                                      echo "<br>Erreur de chargement de la page précédente.";
                                      echo "<br>Location: http://www.plafond-tendu.net/atom/test5.php?action=generation&index=".$index."";
                                      echo '<script language="Javascript">
                                      <!--
                                      document.location.replace("http://www.plafond-tendu.net/atom/test5.php?action=generation&index='.$index.'");
